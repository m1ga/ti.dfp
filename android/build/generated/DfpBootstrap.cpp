...
static Handle<Value> Dfp_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Dfp.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::DfpBindings::lookupGeneratedInit(
...
static void Dfp_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Dfp_getBinding)->GetFunction());
}

static void Dfp_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::DfpBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry DfpBinding = {
	"ti.dfp",
	Dfp_init,
	Dfp_dispose
};
...
Java_ti_dfp_DfpBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.dfp", &DfpBinding);
	titanium::KrollBindings::addExternalLookup(&(::DfpBindings::lookupGeneratedInit));
}
