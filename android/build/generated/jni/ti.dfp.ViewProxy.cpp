...
Handle<FunctionTemplate> ViewProxy::getProxyTemplate()
{
	...
	javaClass = titanium::JNIUtil::findClass("ti/dfp/ViewProxy");
	HandleScope scope;
...
, javaClass, nameSymbol);
	...
	// Method bindings --------------------------------------------------------
	DEFINE_PROTOTYPE_METHOD(proxyTemplate, "requestAd", ViewProxy::requestAd);
...
// Methods --------------------------------------------------------------------
Handle<Value> ViewProxy::requestAd(const Arguments& args)
{
	LOGD(TAG, "requestAd()");
	HandleScope scope;
	...
	if (!methodID) {
		methodID = env->GetMethodID(ViewProxy::javaClass, "requestAd", "()V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'requestAd' with signature '()V'";
			LOGE(TAG, error);
